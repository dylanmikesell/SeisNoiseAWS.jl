...
include("/Users/dmikesell/GIT/SeisNoiseAWS/src/SeisNoiseAWS.jl")
...
addprocs()  # start DEFAULT parallel threads

# Loop over days
for col = 2:2#4 # end-1 # skip the first column, which is channel tag
    # Get the date information
    the_date    = names(df_date)[col  ]  # start date
    the_date_p1 = names(df_date)[col+1]  # end date
    ...
    @info("Number of potential station pair correlations:", length(S))

    t1 = now()  # start timeer

    # Do the station pair correlations in parallel for a single day
    pmap(SeisNoiseAWS.xcor, xcor_pairs, fill(fs, numpairs), fill(cc_len, numpairs), fill(freqmin, numpairs), fill(cc_step, numpairs), fill(freqmax, numpairs), fill(maxlag, numpairs), fill(output_dir, numpairs))
